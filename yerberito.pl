...
mostrar_resultado(Lista, Titulo, _) :-
    ...
    send(Dialog, kind, transient),
    send(Dialog, size, size(600, 700)),
    send(Dialog, scrollbars, vertical),  % <-- Aquí activas el scroll vertical

    ( memberchk(Titulo, ['Plantas medicinales', 'Plantas para enfermedad', 'Plantas analgesicas', 'Botiquin de plantas'])
->  % Mostrar imagen y nombre para cada planta
        forall(member(Planta, Lista),
            (
                atom_string(PlantaAtom, Planta),
                atomic_list_concat(['imgs/', PlantaAtom, '.jpg'], RutaImg),
                ( exists_file(RutaImg) ->
                    new(HBox, device),
                    new(Img, bitmap(RutaImg)),
                    send(Img, size, size(10,10)),
                    send(HBox, display, Img, point(0,20)),
                    send(HBox, display, text(Planta, left, font(helvetica, normal, 14)), point(0,0)),
                    send(Dialog, append, HBox)
                ;   send(Dialog, append, text(Planta, left, font(helvetica, normal, 14)))
                )
            )
        );
      % Para otros resultados, usar browser de texto
        new(Browser, browser('Resultados', size(580, 700))),
